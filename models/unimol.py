...
from .transformers11 import TransformerEncoderWithPair
    ...
    def forward(
        ...
        **kwargs
        ...
        for i in range(len(all_repr)):
            ...
            #print(cls_atomic_reprs[i].size(0))
        ...
        return logits,cls_atomic_reprs,cls_repr1
...
class ModelArgs:
    ...
    def __post_init__(self):
        ...
        if self.dt_rank == 'auto':
            self.dt_rank = math.ceil(self.d_model / 32)
...
class Mamba(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.regressor = nn.Linear(args.d_model, 1)  # Regressor outputting a single value
    ...
    def forward(self, input_ids):
        ...
        Args:
            ...
            attention_mask (optional): shape (b, l)   Attention mask tensor
            labels (optional): shape (b, 1)   Ground truth regression labels
    
        Returns:
            output: shape (b, 1) if labels is None, else scalar loss value
        ...
        x = self.norm_f(x)
        #print("Mamba",x.shape)
        output = self.regressor(x.mean(dim=1))  # Taking mean over sequence dimension to get single value
        x_extracted = []
        out1 = []
        for i in range(x.size(0)):
            sample_input_ids = input_ids[i]
            non_zero_indices = torch.nonzero(sample_input_ids).squeeze()
            selected_indices = [j for j in non_zero_indices.tolist() if sample_input_ids[j] in [16, 23, 19,45,15]]
            #print(len(selected_indices))
            sample_features = x[i, selected_indices, :]
            x_extracted.append(sample_features)
        for feature in x_extracted:
            features = feature.mean(dim=0)
            out1.append(features)
        out1=torch.stack(out1, dim=0)
        return output,x_extracted,out1
