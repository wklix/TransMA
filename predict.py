...
class MolPredict(object):
    ...
    def predict(self, data, save_path=None, metrics='none'):
        ...
        if self.visual == True:
            y_prediction, encodings,y_truths,weight_keshi=self.model.evaluate(self.trainer, self.load_model,self.visual)
        ...
        if self.visual  == True:
            return  y_prediction, encodings,y_truths,smiles_data_,weight_keshi
        else:
            return y_pred
